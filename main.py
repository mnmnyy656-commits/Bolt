...
def start(update: Update, context: CallbackContext):
    ...
    update.message.reply_text("👋 أهلاً بك في بوت الروليت:\nاختر من الخيارات:", reply_markup=InlineKeyboardMarkup(keyboard))
...
def create_roulette(update: Update, context: CallbackContext):
    ...
    user_states[user_id] = "awaiting_channel_forward"
    ...
    if state == "awaiting_link_channel":
        try:
            ...
            if member.status in ["administrator", "creator"]:
                update.message.reply_text("✅ تم ربط القناة.")
            else:
        ...
        if update.message.forward_from_chat:
            ...
            if channel.type not in ['channel', 'supergroup']:
                update.message.reply_text("❌ الرجاء إرسال رسالة معاد توجيهها من قناة.")
                return
            try:
                ...
                update.message.reply_text(f"❌ خطأ في التحقق من القناة: {e}")
                return
        ...
        if not text.isdigit() or int(text) < 1:
            update.message.reply_text("❗️ أرسل رقم صحيح.")
            return
        ...
        if text.lower() == "نعم":
            ...
            update.message.reply_text("📢 أرسل معرف قناة الاشتراك الإجباري.")
        else:
...
def post_roulette(update, context, user_id, info):
    ...
    try:
        ...
        data[str(user_id)]["message_id"] = sent.message_id
        save_data(data)
        context.bot.send_message(chat_id=user_id, text="✅ تم نشر السحب.")

    except Exception as e:
        context.bot.send_message(chat_id=user_id, text=f"❌ خطأ في النشر:\n{e}")
...
def join_roulette(update: Update, context: CallbackContext):
    ...
    if force_channel:
        try:
            ...
            query.answer("❗️ تعذر التحقق من الاشتراك.", show_alert=True)
            return
...
def stop_roulette(update: Update, context: CallbackContext):
    ...
    if not roulette or str(sender_id) != owner_id:
        query.answer("❌ لا يمكنك إيقاف السحب.", show_alert=True)
        return
...
def draw_winners(update: Update, context: CallbackContext):
    ...
    if not participants:
        query.answer("❗️ لا يوجد مشاركين.", show_alert=True)
        return

    winners = random.sample(participants, min(len(participants), roulette["winners_count"]))
    roulette["active"] = False
    save_data(data)
...
def main():
    ...
    # تشغيل Flask في الخلفية لمنع توقف البوت
